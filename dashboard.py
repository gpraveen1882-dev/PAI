...
<div style='font-size:0.78rem; color:#E74C3C; margin-top:4px;'><strong>Risk:</strong> Pipeline too indirect — socios uncommitted.</div>
    ...
    with tab1:
        # Build ranking table
        eco_rank = ecosystem.copy()
        total_orgs   = eco_rank.groupby("country").size().reset_index(name="Total Orgs")
        align5_count = eco_rank[eco_rank["align"]==5].groupby("country").size().reset_index(name="# Align-5 Orgs")
        align4_count = eco_rank[eco_rank["align"]>=4].groupby("country").size().reset_index(name="# Align 4–5 Orgs")
        avg_align    = eco_rank.groupby("country")["align"].mean().reset_index(name="Avg Alignment")
        avg_scale    = eco_rank.groupby("country")["scale"].mean().reset_index(name="Avg Scale")
        active_count = eco_rank[eco_rank["status"].isin(["Active","In Conversation"])].groupby("country").size().reset_index(name="Active / In Conv.")

        rank = total_orgs.merge(align5_count, on="country", how="left")\
                         .merge(align4_count, on="country", how="left")\
                         .merge(avg_align,    on="country", how="left")\
                         .merge(avg_scale,    on="country", how="left")\
                         .merge(active_count, on="country", how="left")
        rank = rank.fillna(0)

        # Composite ecosystem score: 40% avg alignment + 30% align-5 count (normalized) + 30% avg scale
        max_a5 = rank["# Align-5 Orgs"].max()
        rank["Ecosystem Score"] = (
            (rank["Avg Alignment"] / 5) * 40 +
            (rank["# Align-5 Orgs"] / max_a5) * 30 +
            (rank["Avg Scale"] / 5) * 30
        ).round(1)

        rank = rank.sort_values("Ecosystem Score", ascending=False).reset_index(drop=True)
        rank.insert(0, "Rank", range(1, len(rank)+1))
        rank["Avg Alignment"] = rank["Avg Alignment"].round(2)
        rank["Avg Scale"]     = rank["Avg Scale"].round(2)
        rank["# Align-5 Orgs"] = rank["# Align-5 Orgs"].astype(int)
        rank["# Align 4–5 Orgs"] = rank["# Align 4–5 Orgs"].astype(int)
        rank["Active / In Conv."] = rank["Active / In Conv."].astype(int)

        st.markdown(f"""<div class='card-gold'>
<strong>How the Ecosystem Score is calculated:</strong>
<span style='font-size:0.82rem; color:{MGREY};'> 40% Average Alignment Score + 30% Number of 5-Alignment Orgs (normalized) + 30% Average Scale Score. Higher = stronger partner ecosystem.</span>
</div>""", unsafe_allow_html=True)

        # Bar chart — ecosystem score
        fig_rank = px.bar(
            rank.sort_values("Ecosystem Score", ascending=True),
            x="Ecosystem Score", y="country", orientation="h",
            color="Ecosystem Score",
            color_continuous_scale=[[0,"#FCE4D6"],[0.4,AMBER],[0.7,TEAL],[1.0,GREEN]],
            labels={"country":"","Ecosystem Score":"Ecosystem Score (out of 100)"},
            title="Country Ecosystem Strength Ranking",
            height=420,
            text="Ecosystem Score"
        )
        fig_rank.update_traces(textposition="outside", textfont=dict(size=11))
        fig_rank.update_layout(plot_bgcolor=WHITE, coloraxis_showscale=False,
            margin=dict(l=0,r=60,t=40,b=0))
        fig_rank.update_xaxes(gridcolor="#F0F0F0", range=[0,105])
        fig_rank.update_yaxes(showgrid=False)
        st.plotly_chart(fig_rank, use_container_width=True)

        # Full ranking table
        st.markdown(f"<div style='font-weight:700; color:{NAVY}; margin-bottom:8px;'>Full Country Ecosystem Ranking</div>", unsafe_allow_html=True)
        st.dataframe(
            rank[["Rank","country","Ecosystem Score","# Align-5 Orgs","# Align 4–5 Orgs",
                  "Avg Alignment","Avg Scale","Total Orgs","Active / In Conv."]].rename(
                columns={"country":"Country"}),
            use_container_width=True, hide_index=True
        )
        st.caption("Ecosystem Score reflects partner quality and density — separate from the country's operational readiness or CD strength. A strong ecosystem does not guarantee results without a strong CD.")
