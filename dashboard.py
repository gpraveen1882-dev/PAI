...
h1 {{ color: {NAVY}; font-size: 1.6rem; margin-bottom: 0.2rem; }}
h2 {{ color: {NAVY}; }}
h3 {{ color: {NAVY}; }}
</style>
...
def page_header(title, subtitle=None, badge=None):
    """Branded page header — no negative margins, pure HTML div."""
    badge_html = (
        f'<span style="background:{GREEN}; color:white; padding:3px 12px; ' +
        f'border-radius:20px; font-size:0.7rem; font-weight:700; letter-spacing:1px;">{badge}</span>'
    ) if badge else ""
    sub_html = (
        f'<div style="font-size:0.75rem; color:#CADCFC; margin-top:2px;">{subtitle}</div>'
    ) if subtitle else ""
    st.markdown(f"""
<div style="
  background: linear-gradient(135deg, {NAVY} 0%, #243B6E 100%);
...
">
  <div style="display:flex; align-items:center; gap:14px;">
    <img src="{LOGO_URI}" height="36"
         style="background:white; padding:4px 8px; border-radius:7px; display:block;"
...
<div style='padding:1.2rem 0.8rem 0.8rem;'>
  <img src="{LOGO_URI}" width="160" style="background:white; padding:6px 12px; border-radius:8px; display:block; margin:0 auto 10px;" alt="MI"/>
  <div style='text-align:center; font-size:0.65rem; color:#CADCFC; letter-spacing:1px;'>
    Growth Strategy Dashboard
  </div>
  <div style='text-align:center; font-size:0.6rem; color:#445; margin-top:2px;'>
    UCLA Anderson SICC · May 2026
  </div>
</div>
<div style='height:2px; background:linear-gradient(90deg,#27AE60,#028090,transparent); margin:0.2rem 0 0.6rem;'></div>
""", unsafe_allow_html=True)
